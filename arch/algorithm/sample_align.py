...
print(sys.path)
import json
from simulation.vertical_sample_align import TensorflowFederatedJob
...
class SampleAlign(TensorflowFederatedJob):
    ...
    def _get_uid_list(self):
        ...
        with open(job.conf_path, "rb") as f:
            conf = json.load(f)
            return [i["field"] for i in conf["dataSet"]]
    ...
    def _get_dataset_url_list(self):
        ...
        with open(job.conf_path, "rb") as f:
            ...
            file_path_list = list()
            for i in dataSet:
                file_path = self.session.query(DataSet.file_path).filter_by(
                    uid=i["uid"]
                ).first()
